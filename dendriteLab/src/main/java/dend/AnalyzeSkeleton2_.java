...
public class AnalyzeSkeleton2_ implements PlugInFilter, DialogListener
{
	...
	public void run(ImageProcessor ip) 
	{
		...
		/*run(pruneIndex, 20, calculateShortestPath, origIP, false,
				verbose);*/
	...
	{
		Roi roi = null;
		return run(pruneIndex, pruneEnds, shortPath, origIP, silent, verbose, roi);
	}
			...
			Roi roi)
	{
		...
		processSkeleton(this.inputImage, null);
		...
		if(bPruneCycles)
		{
			if(pruneCycles(this.inputImage, this.originalImage, AnalyzeSkeleton2_.pruneIndex))
			{
				// initialize visit flags
				resetVisited();
	 ...
	 * @param silent
	 * @param verbose flag to display running information
	 * @param rootBranches points inside these branches are spared from elimination when
	 *            pruning end branches. 
	 *            
	 */
	public SkeletonResult run(
			int pruneIndex,
			...
			ArrayList<Edge> branches)
	{
		...
		if (pruneEnds) 
		{
			pruneEndBranches(this.inputImage, this.taggedImage, branches);
		}
	 ...
	 *
	 */
	private void pruneEndBranches(ImageStack stack, ImageStack taggedImage, ArrayList<Edge> branches)
	{
			...
			double length) 
	{
		...
		for (int t = 0; t < this.numOfTrees; t++)
		{
			...
			if(debug) {
				//System.out.println("Initial number of vertices: " + graph[t].getVertices().size());
				IJ.log("Initial number of vertices: " + graph[t].getVertices().size());
			}
			...
			if(debug) {
				//System.out.println("Final number of vertices: " + graph[t].getVertices().size());
				IJ.log("Final number of vertices: " + graph[t].getVertices().size());
			}
		}
	...
	public void processSkeleton(ImageStack inputImage2, Point rootPoint) 
	{
		...
		if(rootPoint != null)
			this.taggedImage = tagImage(inputImage2, rootPoint);
		else
	...
	{
		return getPixel(this.taggedImage, point.x, point.y, point.z) == AnalyzeSkeleton2_.SLAB;
	}
	...
	{
		return getPixel(this.taggedImage, point.x, point.y, point.z) == AnalyzeSkeleton2_.JUNCTION;
	}
	...
	{
		return getPixel(this.taggedImage, point.x, point.y, point.z) == AnalyzeSkeleton2_.END_POINT;
	}
	...
	{
		return getPixel(this.taggedImage, x, y, z) == AnalyzeSkeleton2_.JUNCTION;
	}
			...
			for (int x = 0; x < width; x++) 
				for (int y = 0; y < height; y++)
				{
					if(getPixel(inputImage2, x, y, z) != 0)
					{
						...
						if(numOfNeighbors < 2)
						{
							setPixel(outputImage, x, y, z, AnalyzeSkeleton2_.END_POINT);
							this.totalNumberOfEndPoints++;
						...
						{
							setPixel(outputImage, x, y, z, AnalyzeSkeleton2_.JUNCTION);
						...
						{
							setPixel(outputImage, x, y, z, AnalyzeSkeleton2_.SLAB);
	 ...
	 *  
	 * @param inputImage2 skeleton image to be tagged
	 * @param rootPoint - point to designate as new root junction if one doesn't exist
	 * @return tagged skeleton image
	 */
	private ImageStack tagImage(ImageStack inputImage2, Point rootPoint) 
	{
		...
		setPixel(outputImage, rootPoint.x, rootPoint.y, rootPoint.z, AnalyzeSkeleton2_.JUNCTION);
	...
	private void createSettingsDialog() {
		...
		settingsDialog.addChoice("Prune cycle method: ", AnalyzeSkeleton2_.pruneCyclesModes,
